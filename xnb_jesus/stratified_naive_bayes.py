...
class Stratified_NB():
    ...
    def _calculate_bandwidth(self, X:pd.DataFrame, y:pd.Series) -> None:
        inicio = time.time()
        progressBar = Bar('PROG. BANDWIDTH:', max=len(X.columns)*len(self._class_values))
    ...
    def _calculate_kde(self, X:pd.DataFrame, y:pd.Series) -> None:
        ...
        for c in self._class_values:
            ...
            for v in cols:
                ...
                kde = KernelDensity(kernel=self.kernel, bandwidth=bw, algorithm=self.algorithm ).fit(data.values[:, np.newaxis])
    ...
    def _calculate_feature_selection(self):
        inicio = time.time()
        progressBar = Bar('PROG. FEATURE SELECTION:', max=len(self._ranking_divergence))
        threshold = 0.999
        ...
        def addDict(dict_result, class_1, class_2, variable, hellinger, finished_class):
            if not finished_class[class_1][class_2]:
                k = class_1+' || '+variable
                not_in_dict = class_1 not in set(map(lambda x: x.split(' || ')[0], dict_result[class_2].keys()))
                if not_in_dict:
                    dict_result[class_2][k] = hellinger
                    finished_class[class_1][class_2] = hellinger >= threshold
                else:
                    class_list = list(map(lambda x: x.split(' || ')[0], dict_result[class_2].keys()))
                    p = 1
    ...
    def _calculate_necessary_kde(self, X:pd.DataFrame, y:pd.Series) -> None:
        inicio = time.time()
        progressBar = Bar('PROG. NECESSARY KDE:', max=len(self.feature_selection_dict))
        ...
        for _, row in X.iterrows():
            ...
            for c, variables in self.feature_selection_dict.items():
                ...
                # We get the probabilities with KDE. Instead of x_sample (50) records, we pass this time only one
                    k = self._kernel_density_dict[c][v].score_samples(np.array([row[v]])[:, np.newaxis])[0]
                    pr = pr + k
                probability = pr + np.log(self._class_representation[c]) # The last operand is the number of times a record with that class is given in the train dataset
                s += probability
