...
class Stratified_NB():
  ...
  def _kde_lambda(self, c: str, v: str) -> KDE:
    ...
    bw = self._bw[(self._bw.variable == data.name) & (
        self._bw.target == c)].bandwidth.values[0]
    kde = KernelDensity(kernel=self.kernel, bandwidth=bw).fit(
  ...
  def _normalize(self, data: list) -> list:
    s = sum(data)
    # range[0, 1] -> the sum is 1
    return list(map(lambda x: x/s if s != 0 else 0, data))
  ...
  def predict(self, X: pd.DataFrame) -> list:
    ...
    if len(self.feature_selection_dict) == 0:
      raise NotFittedError(
          "This Stratified Naive Bayes instance is not fitted yet. Call 'fit' with appropiate arguments before using this estimator.")
    ...
    for _, row in X.iterrows():
      ...
      for c, variables in self.feature_selection_dict.items():
        ...
        for v in variables:
          ...
          k = mp.exp(kde_dict[v][c].score_samples(
              np.array([mp.mpf(str(row[v]))])[:, np.newaxis])[0])
          kde_values.append(k)

        pr = prod(kde_values)
        # The last operand is the number of times a record with that class is given in the train dataset
        probability = pr * self._class_representation[c]
      ...
      if s > 0:
        # The probability of being the class
